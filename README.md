...
the exif data contains an offset (`OffsetTimeOriginal`/`OffsetTime`) or a
`GPSDateTime` value, which is UTC by definition; otherwise it is naive.
...
an offset or GPS time. With `USE_TZ = True`, wrap the value with
